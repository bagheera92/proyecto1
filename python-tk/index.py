...
class Product:
   ...
   def __init__(self, window):
      self.wind = window
      self.wind.title('Just Control de Stock')

      ## Creating a Frame Container
      frame = LabelFrame(self.wind, text = 'Registro de Producto Nuevo')
      ...
      ttk.Button(frame, text = 'Grabar', command = self.add_product).grid(row = 3, columnspan = 2, sticky = W + E)
   ...
   def add_product(self):
      ...
      if self.validation():
         ...
         self.message['text'] = 'Product {} Insertado satisfactoriamente'.format(self.name.get())
         self.get_products()
      else:
         self.message['text'] = 'Producto y Precio requerido'
   ...
   def delete_product(self):
      ...
      try:
         ...
         self.message['text'] = 'Por favor seleccionar Producto'
         return
   ...
   def edit_product(self):
      ...
      #Old name
      Label(self.edit_wind, text = 'Anterior: ').grid(row = 0, column = 1)
   ...
   def edit_records(self, new_name, name, new_price, old_price):
      ...
      self.message['text'] = 'Record {} Guardado Satisfactoriamente'.format(name)
      self.get_products()
